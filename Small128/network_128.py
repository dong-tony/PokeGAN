...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            nn.ConvTranspose2d(768, 384, 5, stride = 2, padding = 0),
            nn.BatchNorm2d(384),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(384, 256, 5, stride=2, padding=0),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(256, 192, 5, stride=2, padding=0),
            nn.BatchNorm2d(192),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(192, 64, 5, stride=2, padding=0),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(64, 3, 8, stride=2, padding=0),
            nn.Sigmoid()
        )
    def forward(self, x):
        x = x.view(-1, 110)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        # discriminator fc
        self.fc_dis = nn.Linear(512*13*13, 1)
        # aux-classifier fc, 18 different types
        self.fc_aux = nn.Linear(512*13*13, 18)

    def forward(self, x):
        x = self.model(x)
        flattened = x.view(-1, 512*13*13)
